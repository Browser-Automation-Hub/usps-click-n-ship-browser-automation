/**
 * anchor-cloud.js — AnchorBrowser cloud example for USPS Click-N-Ship
 *
...
const { login_usps, create_shipping_label } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting USPS Click-N-Ship automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_usps(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_shipping_label(page, {});
