/**
 * auth.js — Authentication handler for USPS Click-N-Ship
 * Supports: SSO (USPS Account), MFA (USPS Email OTP)
 *
...
const BASE_URL = process.env.USPS_CNS_URL || 'https://cns.usps.com/labelInformation.shtml';
const USERNAME = process.env.USPS_CNS_USERNAME;
const PASSWORD = process.env.USPS_CNS_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set USPS_CNS_USERNAME and USPS_CNS_PASSWORD in .env`);
  }

  console.log('Logging in to USPS Click-N-Ship...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
