# Contributing to USPS Click-N-Ship Browser Automation
...
- **Broken selectors**: USPS Click-N-Ship updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real USPS Click-N-Ship instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.USPS_CNS_URL}/path/to/section`);
...
When you find working selectors for USPS Click-N-Ship, document them:

```javascript
// USPS Click-N-Ship selectors (verified YYYY-MM-DD)
...
USPS Click-N-Ship updates its UI periodically. When selectors break:
